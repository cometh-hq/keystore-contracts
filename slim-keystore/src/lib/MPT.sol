...
library MPT {
    ...
    function verifyStorageSlot(bytes32 root, StorageSlot memory slot, bytes[] memory proof)
        internal
        pure
        returns (bool)
    {
    ...
    function verifyLeaf(bytes32 root, bytes memory key, bytes[] memory proof)
        internal
        pure
        returns (bytes memory result)
    {
        ...
        for (uint256 index = 0; index < proof.length; ++index) {
            ...
            if (node.length == 17) {
                ...
                if (isExtension) root = bytes32(node[1].toUintStrict());
                else break;
            }
        }
    ...
    */
    function checkEncodedPath(bytes memory prefix, bytes memory key, uint8 nibble, uint256 index)
        private
        pure
        returns (bool, uint8)
    {
