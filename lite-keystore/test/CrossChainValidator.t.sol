...
contract CrossChainValidatorLZTest is RhinestoneModuleKit, Test {
    ...
    LiteKeyStore internal liteKeyStore;
    ...
    function setUp() public {
        ...
        liteKeyStore = new LiteKeyStore();

        // Deploy cross-chain validator
        validator = new CrossChainValidator(storageVerifier, address(liteKeyStore));
    ...
    function _createMockStorageProofSignature(
        ...
        CrossChainValidator.StorageProofData memory storageProofData = CrossChainValidator.StorageProofData({
            ...
            owner: _owner,
            slotValue: uint256(
                keccak256(abi.encode(address(liteKeyStore), keccak256(abi.encode(_childSafe, LITE_KEY_STORE_OWNERS_SLOT))))
            ),
            account: MPT.Account({
    ...
    function test_InvalidEOAsigLength() public {
        ...
        bytes memory mockSignature = _createMockStorageProofSignature(
            address(liteKeyStore), childSafe, "0x0000000000000000000000000000000000000000000000000000", 64
        );
        userOp.signature = mockSignature;
    ...
    function test_InvalidEOAsig() public {
        ...
        // Create mock storage proof signature
        bytes memory mockSignature =
            _createMockStorageProofSignature(address(liteKeyStore), childSafe, mockECDSAsignature, 64);
        userOp.signature = mockSignature;
